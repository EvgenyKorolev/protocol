...
void app_in_direct::slot_reset_list()
{
    QSortFilterProxyModel* proxy_mod = new QSortFilterProxyModel();
    proxy_mod->setSourceModel(_apdm);
    _apdv->setModel(proxy_mod);
...
void app_in_direct::result()
{
    _result = _apdv->result();
    emit accept();
}
apparaturs app_in_direct::get_result()
{
    return _result;
}
