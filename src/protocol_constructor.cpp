...
QString protocol_constructor::use_adapt(const std::tuple<std::string, std::string, std::string, std::string , std::string>& arg)
{
    ...
    if (std::get<0>(arg) == "_parent_var"){
        QString source = std::get<3>(arg).c_str();
        if (source.left(8) == "_obj_app" || source.left(8) == "_ktp_app" || source.left(8) == "_cp_app"){
        QList<apparaturs*>* tmpp = app_view->model()->data(QModelIndex(), Qt::UserRole).value<QList<apparaturs*>*>();
        return var_adapter::get_app_table(tmpp, source, QString(std::get<4>(arg).c_str()));
        }
