...
pair_plus_editor::pair_plus_editor(std::tuple<QString, QString, QString, QString> arg, QString &lbl, QString &lbl1, QString &lbl2, QString &lbl3, QString &lbl4, int mw, int mw2, int mw3, int mw4)
{
    this->main_obj = arg;
...
std::tuple<QString, QString, QString, QString> pair_plus_editor::result()
{
    return main_obj;
}
void pair_plus_editor::slot_save()
{
    main_obj = std::make_tuple(my_line1->text(), my_line2->text(), my_line3->text(), my_txt->toPlainText());
    emit accept();
}
void pair_plus_editor::set_mask(QRegExpValidator* arg_val, w_line arg_l)
{
    ...
    } else if (arg_l == w_line::three){
        my_line3->setValidator(arg_val);
    }
}
